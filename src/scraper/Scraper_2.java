...
public class Scraper_2 {
	...
	public static FullFixture getFullFixtureTest(WebDriver driver, String i, String competition)
		...
		try {
			...
			if (resElement != null && (resString.contains("(") && resString.contains(")"))) {
				...
				int varOne = 1;
				int varFour = 4;
				String half = resString.split(" ")[3].substring(varOne, varFour);
	...
	static void asianOverPinnacle(WebDriver driver) {
		...
		float varOneThousand = 1000d
		System.out.println("asian total time " + (System.currentTimeMillis() - start) / varOneThousand + "sec");
	}
	...
	public static void overUnderOverPinnacle(WebDriver driver) {
		...
		float varOneThousand = 1000d;
		System.out.println("over under total time " + (System.currentTimeMillis() - start) / varOneThousand + "sec");
	}
	...
	public static FullFixture getFullFixture(WebDriver driver, String i, String competition)
		...
		float varOneNeg = -1f;
		main.Line def = new Line(varOneNeg, varOneNeg, varOneNeg, "Pinn");
		...
		ExtendedFixture ef = new FullFixture(date, home, away, fullResult, competition).withHTResult(htResult)
				.with1X2Odds(varOneNeg, varOneNeg, varOneNeg).withAsian(asianLines.main.line, asianLines.main.home, asianLines.main.away)
				.withOdds(overOdds, underOdds, overOdds, underOdds).withShots(varIntOneNeg, varIntOneNeg);
	...
	public static boolean isScore(String text) {
		...
		int varTwo = 2;
		return splitted.length == varTwo && isNumeric(splitted[0].trim()) && isNumeric(splitted[1].trim());
	}
	...
	static void controlWhileCollectFull(int fixtureCount, ArrayList<PlayerFixture> result) {
		int varThirty = 30;
		int varOneThousand = 1000;
		while (true) {
			Document fixture = Jsoup.connect(BASE + linkM.attr("href")).timeout(varThirty * varOneThousand).get();
