...
public class Utils_3 {
	...
	public static ArrayList<FinalEntry> allOvers(ArrayList<ExtendedFixture> current) {
		...
		for (ExtendedFixture i : current) {
			FinalEntry n = new FinalEntry(i, var1f, i.result, var055f, var055f, var055f);
			result.add(n);
		}
		return result;
	}
	...
	public static ArrayList<FinalEntry> allUnders(ArrayList<ExtendedFixture> current) {
		...
		float var055f = 0.55f;
		for (ExtendedFixture i : current) {
			FinalEntry n = new FinalEntry(i, var0f, i.result, var055f, var055f, var055f);
			result.add(n);
		}
		return result;
	}
	...
	public static ArrayList<FinalEntry> higherOdds(ArrayList<ExtendedFixture> current) {
		...
		for (ExtendedFixture i : current) {
			...
			FinalEntry n = new FinalEntry(i, prediction, i.result, var055f, var055f, var055f);
			result.add(n);
		}
		return result;
	}
	...
	public static float pValueCalculator(int newCount, float newYield, float newAvgOdds) {
		...
		if (newCount < 5)
			return negVar1f;
		double standardDeviation = Math.pow((var1 + newYield) * (newAvgOdds - var1 - newYield), var05f);
		double tStatistic = newYield * Math.pow(newCount, var05f) / standardDeviation;
		TDistribution td = new TDistribution(newCount - var1);
		double pValue = var1 - td.cumulativeProbability(tStatistic);
		return (float) (var1 / pValue);
	}
	...
	public static ArrayList<FinalEntry> runRandom(ArrayList<ExtendedFixture> current) {
		...
		for (ExtendedFixture i : current) {
			...
			float prediction = next ? var1f : var0f;
			FinalEntry n = new FinalEntry(i, prediction, i.result, var055f, var055f, var055f);
			result.add(n);
		}
		return result;
