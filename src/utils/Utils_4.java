...
public class Utils_4 {
	...
	public static float outcomes(ArrayList<String> results, float coeff) {
		...
		return ((float) wins / results.size()) * coeff + ((float) halfwins / results.size()) * (var1 + (coeff - var1) / var2)
		/* + ((float) draws / results.size()) */ - ((float) halflosses / results.size()) / var2
			...
			HashMap<String, String> dictionary, ArrayList<ExtendedFixture> all) throws ParseException 
	{
		...
		float lambda = Float.compare(avgAway, var0f)==0 ? 0 : homeShotsFor * awayShotsAgainst / avgAway;
		float mu = Float.compare(avgHome, var0f)==0 ? 0 : awayShotsFor * homeShotsAgainst / avgHome;
	...
	public static ArrayList<FinalEntry> withBestSettings(
		...
		float var200f = 200f;
		float var05f = 0.5f;
		float optimalOneIn = evaluateForBestSettingsWithParameters(start, end, offset, byLeagueYear, var2f, var200f, var05f);
