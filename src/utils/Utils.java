...
public class Utils {
	...
	public static Pair poissonAsianHome(float lambda, float mu, float line, float asianHome, float asianAway) {
		...
		float var0f = 0f;
		float var1f = 1f;
		float varNeg05f = -0.5f;
		float var05f = 0.5f;
		float var025f = 0.25f;
		float varNeg025f = -0.25f;
		float var075f = 0.75f;
		float varNeg075f = -0.75f;
		int var1 = 1;
		int var2 = 2;
		int var0 = 0;
		
		if (Float.compare(fraction, var0f)==var0) {
			float winChance = poissonHome(lambda, mu, -whole);
			float drawChance = poissonExact(lambda, mu, -whole);

			float home = winChance * asianHome + drawChance - (var1f - winChance - drawChance);
			float away = (var1f - winChance - drawChance) * asianAway + drawChance - winChance;
			return Pair.of(home, away);

		} else if (Float.compare(fraction, varNeg05f)==var0) {
			line = whole - var1;
			whole = (int) (line - fraction);
			float winChance = poissonHome(lambda, mu, -whole);

			float home = winChance * asianHome - (var1f - winChance);
			float away = (var1f - winChance) * asianAway - winChance;
			return Pair.of(home, away);

		} else if (Float.compare(fraction, var05f)==var0) {
		...
		} else if (Float.compare(fraction, varNeg025f)==var0) {
			line = whole - var1;
			whole = (int) (line - fraction);
			float winChance = poissonHome(lambda, mu, -whole);
			float drawChance = poissonExact(lambda, mu, -whole);

			float home = winChance * asianHome - drawChance / var2 - (var1f - winChance - drawChance);
			float away = (var1f - winChance - drawChance) * asianAway + drawChance * (var1 + (asianAway - var1) / var2) - winChance;
			return Pair.of(home, away);

		} else if (Float.compare(fraction, var025f)==var0) {
			...
			float home = winChance * asianHome + drawChance * (var1 + (asianHome - var1) / var2) - (var1f - winChance - drawChance);
			float away = (var1f - winChance - drawChance) * asianAway - drawChance / var2 - winChance;
			return Pair.of(home, away);

		} else if (Float.compare(fraction, varNeg075f)==var0) {
			line = whole - var1;
			fraction = line % var1;
			whole = (int) (line - fraction);
			float winChance = poissonHome(lambda, mu, -whole);
		...
		} else if (Float.compare(fraction, var075f)==var0) {
			line = (float) Math.ceil(line);
			fraction = line % var1;
			whole = (int) (line - fraction);
			float winChance = poissonHome(lambda, mu, -whole);
			...
			float home = winChance * asianHome - drawChance / var2 - (var1f - winChance - drawChance);
			float away = (var1f - winChance - drawChance) * asianAway + (var1 + (asianAway - var1) / var2) * drawChance - winChance;
			return Pair.of(home, away);
		} else {
			return Pair.of(-1, -1);
		}
