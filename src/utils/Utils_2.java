...
public class Utils_2 {
	...
	public static void printStats(ArrayList<FinalEntry> all, String name) {
		...
		int var100 = 100;
		float var0f = 0f;
		System.out.println(all.size() + " " + name + " with rate: " + format(var100 * Utils.getSuccessRate(all))
				+ " profit: " + format(profit) + " yield: " + String.format("%.2f%%", var100 * profit / all.size())
				+ ((profit >= var0f && !all.isEmpty()) ? (" 1 in " + format(evaluateRecord(all))) : ""));
	}
	...
	public static float correlation(Integer[] arr1, Integer[] arr2) {
		...
		for (int i = 0; i < arr1.length; i++) {
			...
			sumX2 += Math.pow(arr1[i] - avg1, var02f);
			sumY2 += Math.pow(arr2[i] - avg2, var02f);
		}
	...
	public static void drawAnalysis(ArrayList<FinalEntry> all) {
		...
		System.out.println("Draws when under pr: " + (profitUnder - under) + " from " + under + " "
				+ Results.format((float) (profitUnder - under) * var100 / under) + "%");
		System.out.println("Draws when over pr: " + (profitOver - over) + " from " + over + " "
				+ Results.format((float) (profitOver - over) * var100 / over) + "%");
	}
	...
	public static ArrayList<FinalEntry> shotsRestrict(ArrayList<FinalEntry> finals, HSSFSheet sheet)
		...
		for (FinalEntry fe : finals) {
			...
			if (fe.prediction >= fe.upper && Float.compare(shotsScore, var1f)==var0) {
				shotBased.add(fe);
			} else if (fe.prediction <= fe.lower && Float.compare(shotsScore, var0f)==var0) {
				shotBased.add(fe);
			}
		}
		return shotBased;
	...
	public static Pair positionLimits(ArrayList<FinalEntry> finals, Table table, String type) {
		...
		int var23 = 23;
		for (int i = 1; i < 11; i++) {
			ArrayList<FinalEntry> diffPos = positionRestrict(finals, table, i, var23, type);
	...
	public static float basicSimilar(ExtendedFixture f, HSSFSheet sheet, Table table) throws ParseException {
		...
		ArrayList<ExtendedFixture> lastHomeTeam = filter(f.homeTeam,
				XlSUtils.selectLastAll(sheet, f.homeTeam, var50, f.date), filterHome);
		ArrayList<ExtendedFixture> lastAwayTeam = filter(f.awayTeam,
				XlSUtils.selectLastAll(sheet, f.awayTeam, var50, f.date), filterAway);

		ArrayList<ExtendedFixture> lastHomeHomeTeam = filter(f.homeTeam,
				XlSUtils.selectLastHome(sheet, f.homeTeam, var25, f.date), filterHome);
		ArrayList<ExtendedFixture> lastAwayAwayTeam = filter(f.awayTeam,
				XlSUtils.selectLastAway(sheet, f.awayTeam, var25, f.date), filterAway);
		...
		return var06f * allGamesAVG + var03f * homeAwayAVG + var01f * BTSAVG;
	}
